...
fn emit_string_value(
    ...
    if let Some(n) = s.min_len {
        ...
        out.push(quote! {
            if (#accessor).chars().count() < #n_usize {
                violations.push(::protovalidate_buffa::Violation {
                    field: #field, rule: #rule,
    ...
    if let Some(pat) = &s.pattern {
        ...
        let cache_ident = format_ident!("RE_STRING");
        let field = str_field_path(name_lit, field_number);
    ...
    if let Some(wkr) = s.well_known_regex {
        ...
        if let Some(re) = empty_rule_id {
            ...
            out.push(quote! {
                ...
                } else if !(#fn_path)(::core::convert::AsRef::<str>::as_ref(&(#accessor))) {
                    violations.push(::protovalidate_buffa::Violation {
            ...
            out.push(quote! {
                if !(#accessor).is_empty()
                    && !(#fn_path)(::core::convert::AsRef::<str>::as_ref(&(#accessor)))
                {
                    violations.push(::protovalidate_buffa::Violation {
                        field: #field_b, rule: #rule_b,
                        rule_id: ::std::borrow::Cow::Borrowed(#rid),
    ...
    if !s.in_set.is_empty() {
        ...
        out.push(quote! {
            {
                const ALLOWED: &[&str] = &[ #( #set ),* ];
                if !ALLOWED.iter().any(|candidate| *candidate == ::core::convert::AsRef::<str>::as_ref(&(#accessor))) {
                    violations.push(::protovalidate_buffa::Violation {
                        field: #field, rule: #rule,
    ...
    if !s.not_in_set.is_empty() {
        ...
        out.push(quote! {
            {
                const DISALLOWED: &[&str] = &[ #( #set ),* ];
                if DISALLOWED.iter().any(|candidate| *candidate == ::core::convert::AsRef::<str>::as_ref(&(#accessor))) {
                    violations.push(::protovalidate_buffa::Violation {
                        field: #field, rule: #rule,
    ...
    if let Some(prefix) = &s.prefix {
        ...
        out.push(quote! {
            if !(#accessor).starts_with(#prefix) {
                violations.push(::protovalidate_buffa::Violation {
                    field: #field, rule: #rule,
    ...
    if let Some(not_contains) = &s.not_contains {
        ...
        out.push(quote! {
            if (#accessor).contains(#not_contains) {
                violations.push(::protovalidate_buffa::Violation {
                    field: #field, rule: #rule,
